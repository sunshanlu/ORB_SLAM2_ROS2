...
public:
  ...
  /// 计算Sim3变换矩阵
  static int OptimizeSim3(KeyFramePtr pCurr, KeyFramePtr pMatch, Matches &inLier, Sim3Ret &g2oScm, bool bFixedScale = true);

  /// 优化本质图
  static void optimizeEssentialGraph(const LoopConnection &mLoopConnections, MapPtr mpMap, KeyFramePtr pLoopKf, KeyFramePtr pCurrKf, const int &graphTh,
                                     const KeyFrameAndSim3 &mCorrectedG2oScw, const KeyFrameAndSim3 &mNoCorrectedG2oScw, bool bFixedScale = true);

  /// 全局BA优化
  static void globalOptimization(const std::vector<KeyFramePtr> &vpKfs, const std::vector<MapPointPtr> &vpMps, int mnIteration, bool *bStopFlag,
                                 bool bUseRk = false);
