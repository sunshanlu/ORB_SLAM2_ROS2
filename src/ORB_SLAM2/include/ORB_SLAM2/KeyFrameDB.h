...
public:
  ...
  typedef std::shared_ptr<KeyFrameDB> SharedPtr;
...
private:
  /// 获取关键帧和相同单词数目的数据库信息
  void getKfAndWordDB(VirtualFramePtr pFrame, KfAndWordDB &kfAndWordNum, const std::set<KeyFramePtr> &ignoreKfs = std::set<KeyFramePtr>());
