...
public:
    ...
    /// Tracking部分的构造函数
    Tracking(Map::SharedPtr pMap, KFrameDBPtr pKfDB, VocPtr pVoc, const std::string &sBriefTemplate, int nFeatures,
             int nInitFeatures, int nInitFAST, int nMinFAST, int nDepthTh, int nMaxFrame, int nMinFrame, int nColorType,
             bool bOnlyTracking, float fDepthScale, int nLevels, float fScaleFactor);
