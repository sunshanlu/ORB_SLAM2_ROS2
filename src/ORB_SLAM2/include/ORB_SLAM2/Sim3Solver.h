...
public:
  ...
  /// Sim3Solver的工厂模式
  static SharedPtr create(KeyFramePtr pKfp, KeyFramePtr pKfq, const std::vector<cv::DMatch> &pqMatches, std::vector<bool> &vbChoose, bool bFixScale = true,
                          int nMinSet = 3, int nMaxIterations = 100, float fRatio = 0.4, float fProb = 0.99)
  {
...
private:
  /// Sim3Solver的构造函数
  Sim3Solver(KeyFramePtr pKfp, KeyFramePtr pKfq, const std::vector<cv::DMatch> &pqMatches, std::vector<bool> &vbChoose, bool bFixScale = true, int nMinSet = 3,
             int nMaxIterations = 300, float fRatio = 0.4, float fProb = 0.99);

  /// 判断内外点分布
  int checkInliers(std::vector<std::size_t> &vnInlierIndices, const Sim3Ret &modelRet) override;

  /// 计算去质心坐标
  void computeRCenter(const std::vector<cv::Mat> &vP, const std::vector<cv::Mat> &vQ, cv::Mat &P, cv::Mat &Q, cv::Mat &Op, cv::Mat &Oq);
