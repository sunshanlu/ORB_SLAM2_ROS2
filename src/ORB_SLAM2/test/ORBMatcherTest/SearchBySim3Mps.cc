...
int main() {
    ...
    bool ret = solver->iterate(100, Scm, bNoMore, vInlierIndices);
    std::cout << std::endl << "使用自定义的Sim3算法计算的相机位姿" << std::endl;
    std::cout << Scm.mRqp << std::endl;
    std::cout << Scm.mtqp << std::endl;
    ...
    cv::Mat Rmw, tmw;
    pkFrame0->getPose(Rmw, tmw);
    Sim3Ret Smw(Rmw, tmw, 1.0f);
    Sim3Ret Scw = Scm * Smw;

    auto vLoopGroupMps = pkFrame0->getMapPoints();
    std::vector<MapPoint::SharedPtr> vMatchedMps(pkFrame1->getLeftKeyPoints().size(), nullptr);
    for (const auto &m : newMatches)
        vMatchedMps[m.queryIdx] = vLoopGroupMps[m.trainIdx];

    int nM = matcher.searchBySim3(pkFrame1, vLoopGroupMps, vMatchedMps, Scw, 5);

    auto &kps0 = pFrame0->getLeftKeyPoints();
    auto &kps1 = pFrame1->getLeftKeyPoints();
    std::vector<cv::KeyPoint> kps0Show, kps1Show;
    int jdx = 0;
    std::vector<cv::DMatch> matchesShow;
    for (std::size_t idx = 0; idx < vMatchedMps.size(); ++idx) {
        auto &pMp = vMatchedMps[idx];
        if (pMp) {
            kps0Show.push_back(kps0[pMp->getObservation().at(pkFrame0)]);
            kps1Show.push_back(kps1[idx]);
            matchesShow.emplace_back(jdx, jdx, 0);
            ++jdx;
        }
    }

    ORBMatcher::showMatches(pFrame1->getLeftImage(), pFrame0->getLeftImage(), kps1Show, kps0Show, matchesShow);
