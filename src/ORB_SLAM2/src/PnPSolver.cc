 ...
 * @param tcw           输出的相机位移向量
 */
void PnPSolver::ICP(const std::vector<cv::Point3f> &ctlPointsW, const std::vector<cv::Point3f> &ctlPointsC,
 ...
 */
cv::Mat PnPSolver::computeAlpha(const std::vector<cv::Point3f> &vMapPoints,
                                const std::vector<cv::Point3f> &vCtlPoints) {
    int n = vMapPoints.size();
...
void PnPSolver::GLOptimize(const cv::Mat &L, cv::Mat &vbeta, const cv::Mat &rho, int maxIteration) {
    ...
    for (int idx = 0; idx < maxIteration; ++idx) {
        ...
        cv::Mat fullBeta = (cv::Mat_<float>(10, 1, CV_32F) << std::pow(beta1, 2), std::pow(beta2, 2),
                            std::pow(beta3, 2), std::pow(beta4, 2), beta1 * beta2, beta1 * beta3, beta1 * beta4,
                            beta2 * beta3, beta2 * beta4, beta3 * beta4);
        cv::Mat deltaBeta;
...
float PnPSolver::computeDet3(const cv::Mat &mat) {
    return mat.at<float>(0, 0) * mat.at<float>(1, 1) * mat.at<float>(2, 2) +
           mat.at<float>(0, 1) * mat.at<float>(1, 2) * mat.at<float>(2, 0) +
           mat.at<float>(0, 2) * mat.at<float>(1, 0) * mat.at<float>(2, 1) -
